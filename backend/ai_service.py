...
def _mock_questions(user_info: dict) -> list[dict]:
    ...
    # 每个维度随机选一组
    questions = [
        (random.choice(q1), random.choice(q2), random.choice(q3), random.choice(q4), random.choice(q5))
        for q1, q2, q3, q4, q5 in [pool]  # 每个行业只有一组5题（但可扩展为多组）
    ][0]

    return [{"question": q, "options": opts} for q, opts in questions]
